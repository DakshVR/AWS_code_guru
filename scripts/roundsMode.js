...
function updateSGS() {
  GlobalRoundSGS.value =
    GlobalRoundStrokes.valueAsNumber +
    GlobalRoundMinutes.valueAsNumber +
    ":" +
...
function writeRoundToTable(row, rIndex) {
  row.innerHTML =
    "<td>" +
    GlobalUserData.rounds[rIndex].date +
    "</td><td>" +
    GlobalUserData.rounds[rIndex].course +
    "</td><td>" +
    GlobalUserData.rounds[rIndex].SGS +
    " (" +
    GlobalUserData.rounds[rIndex].strokes +
    " in " +
    GlobalUserData.rounds[rIndex].minutes +
    ":" +
    GlobalUserData.rounds[rIndex].seconds +
    ")</td>" +
    "<td><button aria-label='View and Edit Round'" +
    "onclick='editRound(" +
    GlobalUserData.rounds[rIndex].roundNum +
...
function updateRoundInTable(rowIndex) {
  const thisRound = document.getElementById(
    "r-" + GlobalUserData.rounds[rowIndex].roundNum
  );
...
function editRound(roundId) {
  ...
  for (
    GlobalRoundIndex = 0;
    GlobalRoundIndex < GlobalUserData.rounds.length;
    ++GlobalRoundIndex
  ) {
  ...
  //Display dialog
  transitionToDialog(
    GlobalRoundsModeDialog,
    "SpeedScore: Edit Round",
    prepEditRoundForm
  );
}
...
GlobalLogRoundForm.addEventListener("submit", function (e) {
  ...
  let courseValid =
    !GlobalRoundCourse.validity.tooLong &&
    !GlobalRoundCourse.validity.valueMissing;
  //Is the password field valid?
  let strokesValid =
    !GlobalRoundStrokes.validity.typeMismatch &&
  ...
  let minutesValid =
    !GlobalRoundMinutes.validity.typeMismatch &&
  ...
  let secondsValid =
    !GlobalRoundSeconds.validity.typeMismatch &&
...
function keyDownRoundDialogFocused(e) {
  ...
  if (
    e.code === "Tab" &&
    document.activeElement == GlobalFirstFocusableLogRoundItem.get() &&
    e.shiftKey
  ) {
  ...
  if (
    e.code === "Tab" &&
    document.activeElement == GlobalRoundsModeLogCancelBtn &&
    !e.shiftKey
  ) {
